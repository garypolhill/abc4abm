...
my %split = ('split1U'
	     ...
	     'nosplitU' => [["ERROR $urban"]],
	     'nosplitR'
	     => [["ERROR $rural",
		  "SURVEYQUERY \"A SEQ rural\""]]
	    );

$split{'splitU'} = [$split{'split1U'}->[0], $split{'split2U'}->[0]];
$split{'splitUR'} = [$split{'nosplitU'}->[0], $split{'nosplitR'}->[0]];
...
while($ARGV[0] =~ /^-/) {
  ...
  if($opt eq '-prep') {
    ...
    $mode = shift(@ARGV);
    if(!defined($split{$mode})) {
      die "Split mode $mode not defined. Options are ",
      join(", ", sort(keys(%split))), "\n";
    }
  }
  else {
...
for(my $i = 0; $i <= $#reps; $i++) {
  my $repdir = (scalar(@reps) == 1) ? $rundir : "${rundir}-splitX${i}X";

  my $cmd = "$runcmd ";
  my $calib;

  foreach my $config (@{$reps[$i]}) {
    if($config =~ /^ERROR (.*)$/) {
      $calib = $1;
    }
    else {
  ...
  $cmd .= "-config RUNDIR $repdir -config REP $rep -config PREP $prep"
    ." $genefile";
  ...
  # Get all of the XML files we are going to use for this rep

  opendir(CWD, ".");
  while(my $file = readdir(CWD)) {
    next if $file =~ /^\./;
    if(substr($file, 0, length($repdir)) eq $repdir && -d "$file") {
      opendir(SUB, $file);
    ...
    elsif(substr($file, 0, length($repdir)) eq $repdir && $file =~ /\.txt$/) {
      push(@files, "$file");
    }
  }
  closedir(CWD);
...
my %written_headings;
foreach my $calib (keys(%xmls)) {

  my $exe = "$errcmd $calib ".join(" ", @{$xmls{$calib}});
  ...
  if(scalar(keys(%written_headings)) == 0) {
    print CSV "rundir";
    $written_headings{"rundir"} = 1;
    
    foreach my $key (sort(keys(%sample))) {
      $key =~ s/\W/./g;		# Make the name of the key R friendly
      print CSV ",$key";
      $written_headings{$key} = 1;
    }

    if(scalar(@reps) > 1) {
      print CSV ",split";
      $written_headings{"split"} = 1;
    }

    print CSV ",rep,prep";
    $written_headings{"rep"} = 1;
    $written_headings{"prep"} = 1;
    
    foreach my $col (@headings) {
      if($col =~ /\.error$/ || defined($paramRerr{$col})) {
	print CSV ",$col";
	$written_headings{$col} = 1;
      }
    }
    print CSV "\n";
  }
    ...
    foreach my $col (@headings) {
      if($col =~ /\.error$/ || defined($paramRerr{$col})) {
	if(!defined($written_headings{$col})) {
	  die "Mismatch in headings across calibration error computations\n";
	}
      }
    }
  }
  ...
  while(my $line = <ERR>) {
	...
	if(defined($paramRerr{$headings[$i]})) {
	  if(($cols[$i] == 0 && 
	      abs($sample{$paramRerr{$headings[$i]}}) > 0.0001)
	     || ($cols[$i] != 0 && 
