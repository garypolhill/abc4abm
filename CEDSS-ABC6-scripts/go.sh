...
SAMPLES=( \
  "-mode $SPLIT -null" \
  "-mode $SPLIT -pnull bioboost" \
  "-mode $SPLIT -pnull credit" \
  "-mode $SPLIT -pnull habitadjust" \
  "-mode $SPLIT -pnull maxlinks -pnull visits -pnull newsubcatapp -pnull newsubcatstep" \
  "-mode $SPLIT -pnull hedonism1 -pnull hedonism2" \
  "-mode $SPLIT -pnull egoism1 -pnull egoism2" \
  "-mode $SPLIT -pnull biospherism1 -pnull biospherism2" \
  "-mode $SPLIT -pnull frame1 -pnull frame2" \
  "-mode $SPLIT -pnull planning1 -pnull planning2" )
n=${#SAMPLES[@]}
