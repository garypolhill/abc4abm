...
if(scalar(@ARGV) != 16) {
  die "Incorrect number of arguments (", scalar(@ARGV), ")\n",
  "\t", join("\n\t", @ARGV), "\n\n",
  "Usage: $0 [-setting <setting> <value> ...]\n",
...
sub write_sh {
  ...
  print FP "cat \"$xml\" | sed -e \"s/-output\\.csv/-output-\$run.csv/\" > ",
    "\$xml\n";
  print FP "cd \"$netlogo\"\n";
  print FP "$java -Xmx\$mem -XX:ParallelGCThreads=1 -Dfile.encoding=UTF-8 ",
    "-cp NetLogo.jar org.nlogo.headless.Main --threads 1 --model \"",
    "\$wd/$model\" --setup-file \"\$wd/\$xml\" --experiment \"$experiment\" ",
    "> \"\$wd/\$out\" 2> \"\$wd/\$err\"\n";
